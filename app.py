...
st.caption("Vključuje prispevke (DO, OZP), seniorsko olajšavo in progresivno lestvico 2026.")
...
with st.sidebar:
    ...
    st.divider()
    vrsta_izplacila = st.radio("Način izplačila iz 2. stebra:", ["Mesečna renta", "Enkratni odkup"])
    
    if vrsta_izplacila == "Mesečna renta":
        renta_znesek = st.number_input("Mesečna renta [€]", min_value=0.0, value=100.0)
        renta_letna = renta_znesek * 12
    else:
        renta_letna = st.number_input("Znesek enkratnega odkupa [€]", min_value=0.0, value=20000.0)
        renta_znesek = 0
    ...
    seniorska_olajsava = 1500.00 # Informativni znesek za 2026
    ozp_mesecni = 35.00
    do_stopnja = 0.01 # 1% za dolgotrajno oskrbo

# 3. LOGIKA IZRAČUNA
def izracunaj_vse(pok_m, r_letna, r_mesecna, splosna, sen_ol, je_sen, tip):
    # Letne bruto vrednosti
    pok_letna = pok_m * 12
    skupni_bruto_dohodek = pok_letna + r_letna
    
    # PRISPEVKI (znižujejo davčno osnovo)
    prispevek_do = skupni_bruto_dohodek * 0.01
    prispevek_ozp = 35.0 * 12
    
    # DAVČNA OSNOVA (Renta 50%, Odkup 100%)
    renta_v_osnovi = r_letna * 0.5 if tip == "Mesečna renta" else r_letna
    
    # Neto davčna osnova = Bruto - Prispevki - Splošna Olajšava
    neto_osnova = max(0.0, (pok_letna + renta_v_osnovi) - prispevek_do - prispevek_ozp - splosna)
    if je_sen:
        neto_osnova = max(0.0, neto_osnova - sen_ol)
...
rez = izracunaj_vse(pok_mesecna, renta_letna, renta_znesek, splosna_olajsava, seniorska_olajsava, je_seniorka, vrsta_izplacila)

# 4. PRIKAZ REZULTATOV
st.subheader("1. Razčlenitev dohodnine po razredih")
df_raz = pd.DataFrame(rez['razclenitev'], columns=["Razred", "Osnova [€]", "Davek [€]"])
st.table(df_raz.style.format({"Osnova [€]": "{:,.2f}", "Davek [€]": "{:,.2f}"}))

st.subheader("2. Podroben letni razrez (Tabela)")
tabela_rows = [
    ["Bruto pokojnina (Letno)", f"{rez['pok_letna']:,.2f} €"],
    ["Znesek iz 2. stebra (Renta/Odkup)", f"{rez['renta_letna']:,.2f} €"],
    ["Vstop v davčno osnovo", f"{rez['renta_v_osnovi']:,.2f} €"],
    ["Prispevek za dolgotrajno oskrbo (1%)", f"-{(rez['pok_letna']+rez['renta_letna'])*0.01:,.2f} €"],
    ["Zdravstveni prispevek (OZP)", f"-{35.0*12:,.2f} €"],
    ["Splošna davčna olajšava", f"-{splosna_olajsava:,.2f} €"]
]
if je_seniorka:
    tabela_rows.append(["Seniorska olajšava", f"-{seniorska_olajsava:,.2f} €"])
...
st.divider()
if rez['poracun'] > 0:
    st.error(f"⚠️ **DOPLAČILO:** Ob poračunu boste morali doplačati še **{rez['poracun']:,.2f} €**.")
else:
    st.success(f"✅ **VRAČILO:** FURS vam bo vrnil **{abs(rez['poracun']):,.2f} €** preveč plačane akontacije.")
